...
public class JoystickExtreme extends IJoystick {
	...
	public JoystickExtreme(int port) {
		super(port);
		add("DriveX", new ValueUsage(Axis.STICK_Z, false));
		add("DriveY", new ValueUsage(Axis.STICK_Y, true));
		add("Feed", new ValueUsage(Keys.THUMB_TRIGGER, false));
		add("Shoot", new ValueUsage(Keys.INDEX_TRIGGER, false));
	}
